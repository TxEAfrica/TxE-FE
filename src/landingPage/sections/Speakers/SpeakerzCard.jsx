const SpeakerzCard = ({ speaker, position, animationDirection }) => {
	...
	return (
		<div
			...
			<div
				className={` ${position === 2 ? "text-white" : "text-white"} h-full`}>
				<div
					className={`relative ${
			...
			<div
				className={`absolute right-0 ${
					position === 2
						? "-bottom-16 w-56 h-56 lg:w-72 lg:h-72"
						: "-bottom-5 w-48 h-48"
				}`}>
				<div
					className={`absolute inset-0 bg-gradient-to-l from-transparent  ${
						position === 2 ? "via-orange-600/40" : "via-blue-600/40"
			...
			<img
				...
				loading="lazy"
				className={`${
					position === 2 ? "w-4/6" : "w-2/3 md:w-4/5"
